...
/// \brief 窗口(toy)程序入口
/// \param hInstance 应用程序当前句柄
/// \param hPrevInstance 应用程序先前句柄2
/// \param szCmdLine 应用程序命令行字符串指针
/// \param iCmdShow 指明窗口如何显示
/// \return msg.wParam
...
/// \param hWnd 消息类型
/// \param iMsg 特定的消息
/// \param wParam 特定的其它消息(WORD)
/// \param lParam 特定的其它消息(Long)
/// \return 默认的窗口处理函数DefWindowProc
	...
	case WM_COMMAND: /// 菜单响应
		{
			...
			switch (id) {
			case ID_BUTTON_CHANGE: /// 更换类别(最多三种)
				++current_value;
			...
			case ID_BUTTON_SAVE:/// 保存
				{
					...
					if(getfilename(hwnd,filename,1024,1))
					{
						...
						Edit_GetLine(edit, 0, str, sizeof(str));
						const char *p = str;
						const char* svm_type_str = strstr(p, "-s ");///是否有svm_type
						int svm_type = C_SVC;/// svm默认type为C_SVC
						if(svm_type_str != NULL)
							sscanf(svm_type_str, "-s %d", &svm_type); /// 设定svm 的 type
